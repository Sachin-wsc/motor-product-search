...
async function run() {
    ...
    const matchingSpecs = await db
        .select({
            id: products.id,
            name: products.name,
            acKw: productSpecs.acKw
        })
        .from(productSpecs)
        .innerJoin(products, eq(productSpecs.productId, products.id))
        .where(sql`${productSpecs.acKw} >= ${requiredPower}`);

    console.log("MATCHING PRODUCTS:", matchingSpecs);

    const all = await db
        .select({ id: products.id, name: products.name, acKw: productSpecs.acKw })
        .from(productSpecs)
        .innerJoin(products, eq(productSpecs.productId, products.id));
    ...
    const oldWay = await db
        ...
        .from(productSpecs)
        .innerJoin(products, eq(productSpecs.productId, products.id))
        .where(sql`${productSpecs.acKw} >= ${requiredPower.toString()}`);
